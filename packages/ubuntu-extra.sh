...
install_codex() {
  ...
  if ! npm install -g @openai/codex; then
    ubuntu_extra_warn "Codex CLI install failed"
    return
  fi
  ...
  if command_exists codex; then
    ubuntu_extra_info "Codex CLI installed at $(command -v codex)"
  else
    ubuntu_extra_warn "Codex CLI installed but is not on PATH."
    ubuntu_extra_warn "  If node came from asdf, open a new shell or run: asdf reshim nodejs"
  fi
}
